...
Проект содержит собственную базу данных с продуктами и правилами.
А так же REST API для работы с ними. Кроме того, API предоставляет возможность добавлять новые продукты и правила, запрашивать статистику выдачи персональных предложений пользователям.
...
В качестве источника данных используется отдельная (внешняя) база данных, файл должен располагаться src/main/resources/db/data/transaction.mv.db (поставляется отдельно).

Необходимо задать переменные окружения с вашими секретами:
...
Весь список переменных окружения можно найти в файле src/main/resources/application.properties
